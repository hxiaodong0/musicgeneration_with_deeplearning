...
from keras.layers import Input, Dense, Reshape, Dropout, LSTM, Bidirectional, Embedding
...
def get_notes():
    ...
    for file in glob("Pokemon MIDIs/*.mid"):
...
class GAN():
    def __init__(self, rows):
        ...
        self.seq_shape = (self.seq_length, 128)
        self.latent_dim = 128
        self.disc_loss = []
        self.gen_loss = []
        self.dim1=24*5
    ...
    def build_discriminator(self):
        ...
        model = Sequential()
        model.add(LSTM(128, input_shape=self.seq_shape, return_sequences=True))
        model.add(LSTM(128))
        model.add(Dense(512))
    ...
    def build_generator(self):
        ...
        #model = Sequential()
        #model.add(LSTM(128, input_shape=[self.seq_length, self.latent_dim], return_sequences=True))
        ...
        model.add(LeakyReLU(alpha=0.2))
        model.add(BatchNormalization(momentum=0.8))
        model.add(Dense(units=np.prod(self.seq_shape), activation='tanh'))
        model.add(Reshape(self.seq_shape))
        model.summary()
    ...
    def train(self, epochs, train_data, batch_size=128, sample_interval=50):
        ...
        b=a.shape
        track = pypianoroll.Track(pianoroll=np.squeeze(predictions), program=0, is_drum=False,
                      name='Piano')
        emp0 = np.zeros(self.seq_shape)
        track0 = pypianoroll.Track(pianoroll=emp0, program=0, is_drum=True,
                      name='Drums')
        emp2 = np.zeros(self.seq_shape)
        track2 = pypianoroll.Track(pianoroll = emp2, program=0, is_drum=False,
                      name='Guitar')
        emp3 = np.zeros(self.seq_shape)
        track3 = pypianoroll.Track(pianoroll = emp2, program=0, is_drum=False,
                      name='Bass')
        emp4=np.zeros(self.seq_shape)
        track4 = pypianoroll.Track(pianoroll = emp2, program=0, is_drum=False,
                      name='Strings')
        track_list=[track0,track,track2,track3,track4]
        Multrack=pypianoroll.Multitrack(filename=None, tracks=track_list, tempo=120.0, downbeat=None, beat_resolution=24, name='Piano')
        pypianoroll.write(Multrack, "C:\\Users\\10413\\Desktop\\deep_learning\\project\\midi-lstm-gan-master\\output\\output1.midi")
...
if __name__ == '__main__':
    ...
    for files in listOfFiles:
        ...
        if i>299:
            break
    gan = GAN(rows=20*96)
    gan.train(epochs=200, batch_size=32, train_data=data_train, sample_interval=1)